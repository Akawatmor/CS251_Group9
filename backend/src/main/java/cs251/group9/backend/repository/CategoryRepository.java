...
import cs251.group9.backend.entity.Category;
...
public interface CategoryRepository extends JpaRepository<Category, String> {
    ...
    List<Category> findByCNameContaining(@Param("name") String name);
    
    @Query(value = "SELECT c.* FROM category c JOIN game_category gc ON c.cid = gc.cid WHERE gc.gameID = :gameId", nativeQuery = true)
    List<Category> findCategoriesByGameId(@Param("gameId") Integer gameId);
}
